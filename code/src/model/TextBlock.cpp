...
void TextBlock::layout(Renderer *renderer, Epub *epub, int max_width)
{
  ...
  int n = (int)word_widths.size();
  if (n == 0) return;

  // Heap-allocated DP tables — VLAs on the 8KB Arduino stack overflow for long paragraphs
  ...
  while (idx < n)
  {
    ...
    line_breaks.push_back((uint16_t)idx);
  ...
  for (int i = 0; i < (int)line_breaks.size(); i++)
  {
    int lb = (int)line_breaks[i];
    int total_word_width = 0;
    for (int word_index = start_word; word_index < lb; word_index++)
      total_word_width += word_widths[word_index];

    float spare_space = page_width - total_word_width;
    float actual_spacing = (float)space_width;
    int number_words = lb - start_word;

    if (i != (int)line_breaks.size() - 1 && style == JUSTIFIED && number_words > 1)
    ...
    if (style == CENTER_ALIGN)
      xpos = (spare_space - (number_words - 1) * space_width) / 2.0f;

    for (int word_index = start_word; word_index < lb; word_index++)
    {
      word_xpos[word_index] = (uint16_t)xpos;
