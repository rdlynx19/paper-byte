...
public:
  ...
  // True if at least one text block has content (spans with real words).
  // Use this to skip image-only or parse-failed spine items before layout.
  bool has_text_content() const
  {
    for (auto b : blocks) {
      if (b->getType() == TEXT_BLOCK && !b->isEmpty()) return true;
    }
    return false;
  }
