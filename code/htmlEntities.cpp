...
// Function-local static defers construction to first call so the global
// constructor phase (before setup()) never allocates a 160-entry map on the heap.
static std::unordered_map<std::string, std::string>& get_entity_lookup() {
...
{
  auto &lut = get_entity_lookup();
  auto it = lut.find(entity);
  if (it != lut.end())
  {
    res += it->second;
    return true;
